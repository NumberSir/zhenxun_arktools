...
def ocr(image_url: str) -> set:
    ...
    try:
        cred = credential.Credential(
            Config.get_config("open_recruitment", "TENCENT_CLOUD_SECRET_ID"),
            Config.get_config("open_recruitment", "TENCENT_CLOUD_SECRET_KEY")
        )
        ...
        pre_tags = {word["DetectedText"] for word in data["TextDetections"] if
                    all(c not in word["DetectedText"] for c in filtered_char)}
...
def build_image(result_list: list) -> Image:
    ...
    for data in result_list:
        ...
        for tag_idx, tag in enumerate(tags):
            text_border(text=tag, draw=Draw(tag_bg), x=72, y=102 - 18 * L + 36 * tag_idx, anchor="mm", font=font,
                        shadow_colour=(0, 0, 0, 255), fill_colour=(255, 255, 255, 255))
        ...
        if pri_height == 0:
            ...
            main_background.paste(im=tag_background, mask=tag_background.split()[3],
                                  box=(pri_width + 24, pri_height + 42))
        else:
